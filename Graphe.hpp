          ...
          m_suivant(nullptr)
{
    ...
    POSTCONDITION(m_numero == p_source->m_numero);
    POSTCONDITION(m_listeDest == nullptr);
 ...
 * \post Le graphe n'est pas modifié par cette opération.
 */
template <typename Objet>
...
/**
 * \brief Calcule le plus court chemin entre deux sommets à l'aide de l'algorithme de Dijkstra.
 * \pre Les sommets p_eOrigine et p_eDestination existent dans le graphe.
 * \param[in] p_eOrigine Le sommet d'origine.
 * \param[in] p_eDestination Le sommet de destination.
 * \param[out] p_chemin Un vecteur contenant les sommets du plus court chemin dans l'ordre.
 * \return La distance totale du chemin si un chemin existe, -1 sinon.
 * \throws std::bad_alloc Si la mémoire est insuffisante pour allouer le chemin.
 * \post Le graphe reste inchangé après l'exécution de cette méthode.
 * \post p_chemin contient les sommets du chemin trouvé (ou reste vide si aucun chemin n'existe).
 * \post Les informations des sommets et des étiquettes du graphe initial sont conservées.
 */
template <typename Objet>
int Graphe<Objet>::dijkstra(const Objet& p_eOrigine, const Objet& p_eDestination, std::vector<Objet>& p_chemin) {
    ...
    std::vector<int> sommetsAvant = listerSommets();
    std::vector<Objet> etiquettesAvant = listerEtiquetteSommets();
    ...
    try {
        ...
        copieGraphe._initPathFinding(p_eOrigine, p_eDestination, origine, destination);

        std::vector<Sommet*> Q;
        for (Sommet* sommet = copieGraphe.m_listSommets; sommet != nullptr; sommet = sommet->m_suivant) {
            Q.push_back(sommet);
        }
        ...
        while (!Q.empty()) {
            ...
            std::vector<int> voisins = copieGraphe.listerSommetsAdjacents(u->m_numero);
            for (int v_num : voisins) {
                Sommet* v = copieGraphe._getSommet(v_num);
                if (std::find(Q.begin(), Q.end(), v) != Q.end() && copieGraphe.arcExiste(u->m_numero, v_num)) {
                    int coutArc = copieGraphe.getCoutArc(u->m_numero, v_num);
                    int temp = u->m_cout + coutArc;
        ...
        if (destination->m_predecesseur == nullptr) {
            return -1;
        }

        copieGraphe._recupererChemin(destination, p_chemin);

        POSTCONDITION(!p_chemin.empty());

        // Comparer les sommets du graphe initial et final comme POSTCONDITION "Le graphe original reste inchangé"
        POSTCONDITION(listerSommets() == sommetsAvant);
        POSTCONDITION(listerEtiquetteSommets() == etiquettesAvant);
...
/**
 * \brief Trouve les points d'articulation du graphe
 * \param[out] p_sommets Un vecteur contenant les étiquettes des sommets d'articulation.
 * \throws std::bad_alloc Si la mémoire est insuffisante pour effectuer l'opération.
 * \post Le graphe reste inchangé après l'exécution de cette méthode.
 */
template <typename Objet>
void Graphe<Objet>::getPointsArticulation(std::vector<Objet>& p_sommets) {
    try {
        ...
        for (int numeroSommet : sommets) {
            copieGraphe.enleverSommet(numeroSommet);

            if (!copieGraphe.estFortementConnexe()) {
                p_sommets.push_back(getEtiquetteSommet(numeroSommet));
            }
